...
(function() {
  ...
  function showResult(results) {
    ...
    if (!labels) {
      ...
      var message = 'Probably a Stop Sign';

      if (classifier !== '0')
        message = 'No labels (from ' + classifier + ' grouping) recognized in the image';
        $(".trafficSign").html("Stop Sign Detected");
  ...
  function populateTable(labels) {
    ...
    if(htmlString == 'Person' || 'Human' || 'Winter sports'){
      $(".trafficSign").html("Walk Sign Detected");
      console.log('WALK SIGN DETECTED');
    } else {
      $(".trafficSign").html("Stop Sign Detected");
